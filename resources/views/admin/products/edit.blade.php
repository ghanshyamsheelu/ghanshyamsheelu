...
<div class="card">
    ...
    <?php $imgPath = url('/public/storage/images/').$product->photo; ?>
    <div class="card-body">
        <form action="{{ route("admin.products.update", [$product->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories[]" id="categories" multiple>
                    @foreach($category as $key => $val)
                        <option value="{{ $key }}" {{ (in_array($key, old('categories', [])) || $product->categories) ? 'selected' : '' }}>{{ $val }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                @if ("/storage/images/{{ $product->photo }}")
                    <img src="/storage/images/{{ $product->photo }}" style="width: 50px;">
                @else
                    <p>No image found</p>
                @endif
